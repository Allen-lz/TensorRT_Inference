...
std::vector<float> Model::PreProcess(std::vector<cv::Mat> &vec_img) {
    ...
    for (const cv::Mat &src_img : vec_img) {
        ...
        if (resize == "directly") {
            cv::resize(flt_img, flt_img, cv::Size(IMAGE_WIDTH, IMAGE_HEIGHT));  // 这里使用的就是默认的线性插值
        } else if (resize == "keep_ratio") {
            float ratio = min(float(IMAGE_WIDTH) / float(src_img.cols), float(IMAGE_HEIGHT) / float(src_img.rows));
        ...
        // cv::imshow("show", flt_img);
        // cv::waitKey(0);


        // 不知道直接在这里使用1 / alpha和std有什么区别
        if (INPUT_CHANNEL == 1)
        ...
        if (image_order == "BCHW") {
            ...
            for (int i = 0; i < INPUT_CHANNEL; ++i) {
                ...
                // std::cout << "==Python风格==\n"
                //           << cv::format(split_img[i], cv::Formatter::FMT_PYTHON)
                //           << std::endl;
    ...
    // fread(
    //     result.data(), 1,
    //     BATCH_SIZE * IMAGE_WIDTH * IMAGE_HEIGHT * INPUT_CHANNEL * sizeof(float),
    //     f);
    // fclose(f);
    return result;
}
